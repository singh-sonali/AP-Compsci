...
while i<imgx-15:
	...
	i+=15
bands = int(input("How many gradient bands would you like the picture to contain? *Please enter a power of 2 for symmetry.\n>> "))
#this determines how wide the band is
bandwidth = imgx//bands
#b signifies each band
for b in range (0,bands):
	#col signifies all x values in the bandwidth
	for col in range(0,bandwidth):
		#y ensures the band stretches from the top of the picture to the bottom
		for y in range (0, imgy):
			#scales x values in bandwith to correspond to colors from 255-0
